...
- Images are all standardized to be the same canvas size so that they would show up equally spaced in the application.
- The blanks are updated so that selected blank now blinks in orange so the player more easily recognizes which blank is selected.
### Bug Fixes
